...
if __name__ == '__main__':
    ...
    play_game(board=Board(BOARD, [Player('MiniMax', AI.expictimax)]))

    # Ngames = 5
    # start = time.time()
    # results = []
    # for _ in range(Ngames):
    #     res = play_game(board=Board(BOARD, [Player('MiniMax', AI.expictimax)]), verbose=False)
    #     print(f'({_}) result: {res}.')
    #     results += [res]
    # # # results = [play_game(board=Board(BOARD, [Player('UCT', AI.UCT)]), verbose=False)
    # # #            for _ in range(Ngames)]
    # end = time.time()
    # total = round(end - start, 3)
    # scores = sorted(score for pair in results for score in pair)
    # print(f'min: {min(scores)}, median: {scores[len(scores)//2]}, max: {max(scores)}')
    # print(f'total runtime: {total}, average runtime: {total/Ngames}')
